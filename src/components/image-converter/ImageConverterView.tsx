...
export function ImageConverterView() {
   ...
   useEffect(() => {
      ...
      localStorage.setItem(STORAGE_KEYS.IMAGE_CONVERTER_INPUT, JSON.stringify({
         format,
         quality,
         maintainCropRatio,
         cropRatioPreset
   ...
   const processImage = async () => {
      ...
      try {
         ...
         const compressedFile = await imageCompression(new File([workingBlob], 'temp', { type: workingBlob.type }), options);
         ...
         <div className="w-full md:w-[350px] shrink-0 flex flex-col gap-6">
            ...
            <div className="space-y-4 bg-card rounded-xl border border-border p-5 shadow-sm">
               ...
               <div className="space-y-3 pt-4 border-t border-border/50">
                  <div className="flex items-center justify-between">
                     <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Explicit Resizing</label>
                  ...
                  <p className="text-[10px] text-muted-foreground leading-snug">Leave blank to use the cropped source dimensions directly.</p>
         ...
         <div className="flex-1 flex flex-col bg-card border border-border rounded-xl shadow-sm overflow-hidden min-h-[500px]">
            {!originalPreview ? (
               ...
               >
                  ...
                  <p className="text-sm text-muted-foreground text-center">Drag & drop here, or click to browse files<br/>(PNG, JPEG, WebP)</p>
               </div>
            ) : (
               <div className="flex-1 flex flex-col overflow-hidden">
                  <div className="px-4 py-3 border-b border-border bg-muted/40 flex items-center justify-between shrink-0">
                     <div className="text-sm font-semibold text-foreground flex items-center gap-2">
                        Workspace
                        {processedBlob && <span className="text-xs bg-green-500/10 text-green-500 px-2 py-0.5 rounded-full font-mono">Ready</span>}
                     </div>
                     {processedBlob && (
                     ...
                     {!croppedPreview ? (
                        <div className="w-full flex flex-col items-center justify-center">
                           <div className="mb-4 bg-background/50 border border-border p-1.5 rounded-xl inline-flex gap-2">
                              <ToggleGroup type="single" value={cropRatioPreset} onValueChange={(val) => {
                                 if (val) handleRatioPresetChange(val);
                              }}>
                                 <ToggleGroupItem value="free" size="sm" className="text-xs font-mono px-3">Free</ToggleGroupItem>
                                 <ToggleGroupItem value="1:1" size="sm" className="text-xs font-mono px-3">1:1</ToggleGroupItem>
                                 <ToggleGroupItem value="16:9" size="sm" className="text-xs font-mono px-3">16:9</ToggleGroupItem>
                                 <ToggleGroupItem value="4:3" size="sm" className="text-xs font-mono px-3">4:3</ToggleGroupItem>
                                 <ToggleGroupItem value="21:9" size="sm" className="text-xs font-mono px-3">21:9</ToggleGroupItem>
                              </ToggleGroup>
                           </div>
                           <p className="text-xs text-muted-foreground text-center mb-4">Drag handles to crop the active workspace overlay.</p>
                           <ReactCrop 
                              crop={crop} 
                              onChange={(c) => setCrop(c)} 
                              onComplete={handleCropComplete}
                              aspect={maintainCropRatio ? (
                                 cropRatioPreset === '1:1' ? 1 :
                                 cropRatioPreset === '16:9' ? 16/9 :
                                 cropRatioPreset === '4:3' ? 4/3 :
                                 cropRatioPreset === '21:9' ? 21/9 : undefined
                              ) : undefined}
                           >
                              <img 
                                 ref={imgRef}
                        ...
                        <div className="w-full flex flex-col items-center justify-center">
                           <p className="text-xs text-green-500 text-center mb-4">Processing complete! Preview below.</p>
                           <img 
                              src={croppedPreview} 
                              alt="Result" 
                           ...
                           <Button variant="link" size="sm" onClick={() => setCroppedPreview(null)} className="mt-2 text-xs text-orange-500">
                              Edit Again
                           </Button>
                        </div>
                     )}
