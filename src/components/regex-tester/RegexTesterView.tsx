...
const PATTERN_LIBRARY = [
   {
      ...
      flags: 'g',
      description:
         'Matches standard email addresses like user@domain.com. Validates the local part (before @), domain name, and top-level domain (2+ characters).',
   ...
   },
   {
      ...
      flags: 'g',
      description:
         'Matches US phone numbers in common formats: (123) 456-7890, 123-456-7890, +1 123 456 7890, and more. Handles optional country code, parentheses, dashes, dots, and spaces.',
      example:
   ...
   },
   {
      ...
      flags: 'g',
      description:
         'Matches international phone numbers with optional country code prefix (+91, +44, etc). Supports various separators like dashes, dots, spaces, and parentheses.',
   ...
   },
   {
      ...
      flags: 'gi',
      description:
         'Matches web URLs and domains with optional protocol (http/https), subdomains, and paths. Supports most common TLDs up to 6 characters.',
      example:
         'Visit https://www.example.com/page or http://blog.site.co.uk/post/123\nPlain domain: google.com\nInvalid: ftp://files.com, just-text',
   },
   {
      name: 'IPv4 Address',
      ...
      flags: 'g',
      description:
         'Matches valid IPv4 addresses (four octets 0\u2013255 separated by dots). Each octet is validated to not exceed 255.',
      example:
         'Server IPs: 192.168.1.1, 10.0.0.255, 172.16.254.1\nLoopback: 127.0.0.1\nInvalid: 256.1.1.1, 999.999.999.999',
   },
   {
      name: 'IPv6 Address',
      ...
      flags: 'g',
      description:
         'Matches full IPv6 addresses (eight groups of four hex digits separated by colons). Does not match abbreviated forms with :: shorthand.',
      example:
   ...
   },
   {
      ...
      flags: 'g',
      description:
         'Matches dates in ISO 8601 format (YYYY-MM-DD). Validates month (01-12) and day (01-31) ranges. Does not check for month-specific day limits (e.g., Feb 30).',
      example:
         'Meeting on 2024-01-15 and deadline is 2024-12-31\nBirthday: 1990-06-25\nInvalid: 2024-13-01, 2024-00-15',
   },
   {
   ...
   },
   {
      ...
      flags: 'g',
      description:
         'Matches major credit card numbers: Visa (starts with 4), Mastercard (51-55), Amex (34/37), and Discover (6011/65). Validates length per card type.',
      example:
   ...
   },
   {
      ...
      flags: 'gi',
      description:
         'Matches UUID version 4 strings. The 3rd group always starts with "4" (version) and the 4th group starts with 8, 9, a, or b (variant).',
      example:
   ...
   },
   {
      ...
      flags: 'g',
      description:
         'Matches CSS hex color codes in #RGB or #RRGGBB format. The # prefix is required. Case-insensitive for hex digits A-F.',
   ...
   },
   {
      ...
      flags: 'gi',
      description:
         'Matches CSS color functions: rgb(), rgba(), hsl(), and hsla(). Supports both integer and percentage values.',
      example:
   ...
   },
   {
      ...
      flags: 'm',
      description:
         'Validates strong passwords: minimum 8 characters with at least one lowercase letter, one uppercase letter, one digit, and one special character (@$!%*?&). Each line is tested independently.',
      example: 'P@ssw0rd!\nSecure$1xy\nMyP@ss99\npassword\n12345678\nAbcdefgh\nweak',
   },
   {
      name: 'Username',
      ...
      flags: 'g',
      description:
         'Matches usernames that start with a letter, followed by 2-15 alphanumeric characters, underscores, or hyphens. Total length: 3 to 16 characters.',
   ...
   },
   {
      ...
      flags: 'g',
      description:
         'Matches URL-friendly slugs: lowercase alphanumeric strings separated by single hyphens. Cannot start or end with a hyphen.',
      example:
         'Valid slugs: hello-world, my-blog-post-2024, about-us\nFilename: some-page\nInvalid: -leading, trailing-, double--hyphen',
   },
   {
      name: 'HTML Tag',
      ...
      flags: 'g',
      description:
         'Matches HTML opening, closing, and self-closing tags with optional attributes. Captures tags like <div>, </span>, <img />, and <a href="...">.',
      example:
   ...
   },
   {
      ...
      flags: 'g',
      description:
         'Matches MAC (hardware) addresses in colon-separated or hyphen-separated format. Six groups of two hex digits.',
   ...
   },
   {
      ...
      flags: 'g',
      description:
         'Matches Markdown-style links in the format [text](url). Captures both the link text and the URL/path separately as groups.',
      example:
   ...
   },
   {
      ...
      flags: 'g',
      description:
         'Matches social media-style hashtags: # followed by a letter or underscore, then any combination of letters, digits, and underscores.',
      example:
         'Trending: #JavaScript #web_dev #React2024\nAlso: #100DaysOfCode #AI #open_source\nInvalid: #123 (starts with digit), # (empty)',
   },
];
...
const FLAG_DESCRIPTIONS: Record<string, { name: string; desc: string }> = {
   g: {
      name: 'Global',
      desc: 'Find all matches in the string, not just the first one.',
   },
   i: {
      name: 'Case Insensitive',
      desc: 'Letters match both uppercase and lowercase. "a" matches "A".',
   },
   m: {
      name: 'Multiline',
      desc: 'Makes ^ and $ match the start/end of each line, not just the whole string.',
   },
   s: {
      name: 'Dot All',
      desc: 'Makes the dot (.) match newline characters as well.',
   },
};
...
export function RegexTesterView() {
   ...
   const evaluateRun = useCallback(() => {
      ...
      timeoutRef.current = setTimeout(() => {
         ...
         setResult({
            id: reqId,
            error: 'Execution timeout: Pattern caused catastrophic backtracking or took too long (>2500ms).',
            isTimeout: true,
         });
         setIsEvaluating(false);
      }, 2500);
            ...
            <div className="flex-1 lg:flex-4 flex flex-col min-h-[200px] lg:min-h-0 relative group isolate">
               <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-background/95 backdrop-blur z-20">
                  ...
                  <Button
                     ...
                     size="icon"
                     className="h-6 w-6"
                     onClick={() => copyToClipboard(testString, 'text')}
                  >
                     {copiedField === 'text' ? (
                           ...
                           <div className="divide-y divide-border text-sm font-mono break-all">
                              <div className="grid grid-cols-12 hover:bg-muted/50 transition-colors">
                                 <div className="col-span-12 p-2 px-3 border-l-4 border-l-indigo-500 text-foreground">
                                    <span className="text-indigo-500 mr-2 text-xs font-sans font-medium uppercase min-w-[30px] inline-block">
                                       Full
                                    </span>
                                    {match.fullMatch}
                                 </div>
                              ...
                              {match.groups.map((group, gIdx) => (
                                 <div
                                    key={gIdx}
                                    className="grid grid-cols-12 hover:bg-muted/50 transition-colors text-muted-foreground"
                                 >
                                    <div className="col-span-12 p-1.5 px-3 border-l-4 border-l-emerald-500/50 pl-5 text-[13px]">
                                       <span className="text-emerald-500/70 mr-2 text-[10px] font-sans font-semibold uppercase min-w-[30px] inline-block">
                                          G{gIdx + 1}
                                       </span>
            ...
            <div className="flex-1 lg:flex-2 flex flex-col min-h-[200px] lg:min-h-0 bg-background z-10">
               <Tabs defaultValue="cheatsheet" className="flex flex-col h-full w-full">
                  <TabsList className="w-full justify-start rounded-none border-b border-border h-auto py-0 bg-background/95 backdrop-blur shrink-0 px-2 space-x-2">
                     <TabsTrigger
                        value="cheatsheet"
                        className="py-2.5 data-[state=active]:bg-muted/50 data-[state=active]:shadow-none data-[state=active]:font-semibold rounded-none"
                     >
                           ...
                           <div key={idx} className="p-4 space-y-3">
                              ...
                              <div className="grid gap-2">
                                 {section.items.map((item, i) => (
                                    ...
                                    >
                                       <code className="text-indigo-500 dark:text-indigo-400 font-mono text-xs font-bold px-1.5 py-0.5 bg-indigo-500/10 rounded">
                                          {item.code}
                                       </code>
                                       <span className="text-xs text-muted-foreground mt-0.5 ml-3 flex-1">
                                          {item.desc}
                                       </span>
                                    </button>
                                 ))}
                     ...
                     <div className="space-y-2">
                        {PATTERN_LIBRARY.map((libOpts, i) => {
                           ...
                           return (
                              ...
                              >
                                 <div
                                    className={`text-sm font-semibold mb-1 transition-colors ${isActive ? 'text-indigo-500' : 'group-hover:text-indigo-500'}`}
                                 >
                                    {libOpts.name}
                                 </div>
