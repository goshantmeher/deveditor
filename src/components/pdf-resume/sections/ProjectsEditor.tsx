            ...
            <div key={entry.id} className="relative p-4 rounded-lg border border-border bg-background space-y-3">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="space-y-1">
                     ...
                     <Input value={entry.name} onChange={(e) => updateEntry(i, { name: e.target.value })} placeholder="My Open Source Project" className="h-9 text-sm" />
                  </div>
                  <div className="space-y-1">
                     <Label className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Tech Stack</Label>
                     <Input value={entry.techStack} onChange={(e) => updateEntry(i, { techStack: e.target.value })} placeholder="React, Node.js, PostgreSQL" className="h-9 text-sm" />
                  </div>
                  <div className="space-y-1 md:col-span-2">
                     <Label className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Description</Label>
                     <Textarea value={entry.description} onChange={(e) => updateEntry(i, { description: e.target.value })} placeholder="Brief description of the project and your role..." className="min-h-[60px] text-sm resize-none" />
                  </div>
                  <div className="space-y-1 md:col-span-2">
                     <Label className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Link (Optional)</Label>
                     <Input value={entry.link} onChange={(e) => updateEntry(i, { link: e.target.value })} placeholder="github.com/user/project" className="h-9 text-sm" />
                  </div>
               </div>
            </div>
         ))}
