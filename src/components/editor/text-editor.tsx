...
const TextEditor = React.forwardRef<EditorHandle, TextEditorProps>(function TextEditor(
   { data, onChange, config, comparisonData, onErrorPanelChange, ariaLabel },
   ref
) {
   ...
   // Tracks whether the current data change originated from inside the editor
   ...
   // Clear errors only when data changes from an EXTERNAL source (import, sync)
   // Internal changes (user typing) are flagged via internalChangeRef
   React.useEffect(() => {
      if (!mounted) return;
      if (internalChangeRef.current) {
         // This data change came from our own onChange — don't clear errors
         internalChangeRef.current = false;
         return;
      }
      // External data change — clear stale errors
      setError(null);
   ...
   const parseErrorPosition = React.useCallback(
      (errorMessage: string, source?: string) => parseErrorPosUtil(errorMessage, source),
      []
   );
   ...
   const navigateToError = React.useCallback(() => {
      ...
      try {
         ...
         } else if (typeof errorPosition.line === 'number' && typeof errorPosition.column === 'number') {
   ...
   const fixJson = React.useCallback(() => {
      try {
         ...
         const errorPos = parseErrorPosition(errorMessage, typeof data === 'string' ? (data as string) : undefined);
         if (errorPos) {
            setErrorPosition(errorPos);
         }
         setAutoFixDisabled(true);
               ...
               <div className="flex gap-2">
                  {errorPosition && (
                     <Button onClick={navigateToError} variant="ghost" size="sm">
                        {errorPosition.line ? `Go to Line ${errorPosition.line}` : 'Go to Error'}
                     </Button>
                  )}
                  {!autoFixDisabled && (
   ...
   }, [mounted, error, errorPosition, autoFixDisabled, navigateToError, fixJson, onErrorPanelChange]);
   ...
   }, [mounted, config.formatState, config.formatVersion]);
   ...
   React.useEffect(() => {
      ...
      const id = window.setTimeout(() => {
         ...
         // Flag as internal so the data-change effect doesn't clear errors
         internalChangeRef.current = true;
         onChange(pendingText);
         setPendingText(null);
      }, PARSE_DEBOUNCE_MS);
   ...
   const extensions = useMemo(() => {
      ...
      const labelExtension = EditorView.contentAttributes.of({
         'aria-label': `${ariaLabel || 'JSON code editor'}${config.compareMode ? ' (compare mode)' : ''}`,
         role: 'textbox',
      });
      const baseExtensions = [EditorView.lineWrapping, themeExtension, labelExtension];
   ...
   const gutterExt = useMemo(
      () => (error && errorPosition ? createErrorGutterExtension(errorPosition, error) : null),
      [error, errorPosition]
   );
   ...
   const diffPlugin = useMemo(
      () => (diffLines && config.compareMode ? createDiffDecorationsPlugin(diffLines) : null),
   ...
   const expandAll = React.useCallback(() => {
      const view = editorRef.current?.view;
      if (!view) return;
      // Unfold all foldable regions recursively
      unfoldAll(view);
   }, []);

   // Collapse all sections - custom implementation that folds innermost ranges first
   // so that unfolding a parent keeps children folded
   const collapseAll = React.useCallback(() => {
      const view = editorRef.current?.view;
      if (!view) return;

      // First, unfold everything to get a clean state
      unfoldAll(view);

      // Collect all foldable ranges by iterating line by line
      const { state } = view;
      ...
      // Sort ranges so that innermost (smallest / deepest) ranges come first
      // This ensures children are folded before parents
      ranges.sort((a, b) => {
         // Ranges that are contained within others should be folded first
         // Smaller ranges (inner) should come first
         const sizeA = a.to - a.from;
         const sizeB = b.to - b.from;
         return sizeA - sizeB;
      });

      // Apply all folds in one dispatch - innermost first
