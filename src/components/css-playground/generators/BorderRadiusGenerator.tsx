...
export function BorderRadiusGenerator() {
   ...
   const handleAllChange = (v: number) => {
      setAll(v);
      if (linked) {
         setTopLeft(v); setTopRight(v); setBottomRight(v); setBottomLeft(v);
      }
   };
      ...
      <div className="flex flex-col md:flex-row h-full min-h-0">
         <div className="md:w-[320px] shrink-0 overflow-y-auto border-r border-border/30 p-4 space-y-4">
            ...
            <ControlGroup title="Border Radius">
               ...
               {linked ? (
                  <ControlSlider label="All Corners" value={all} onChange={handleAllChange} min={0} max={200} unit="px" />
               ) : (
                  <>
                     <ControlSlider label="Top Left" value={topLeft} onChange={setTopLeft} min={0} max={200} unit="px" />
                     <ControlSlider label="Top Right" value={topRight} onChange={setTopRight} min={0} max={200} unit="px" />
                     <ControlSlider label="Bottom Right" value={bottomRight} onChange={setBottomRight} min={0} max={200} unit="px" />
                     <ControlSlider label="Bottom Left" value={bottomLeft} onChange={setBottomLeft} min={0} max={200} unit="px" />
                  </>
               )}
            </ControlGroup>

            <ControlGroup title="Border Options">
               <ControlSlider label="Border Width" value={borderWidth} onChange={setBorderWidth} min={0} max={20} unit="px" />
               {borderWidth > 0 && (
                  <ControlColor label="Border Color" value={borderColor} onChange={setBorderColor} />
               )}
            </ControlGroup>
         </div>
