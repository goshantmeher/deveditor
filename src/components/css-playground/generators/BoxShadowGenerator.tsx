...
const DEFAULT_SHADOW: Shadow = { x: 5, y: 5, blur: 15, spread: 0, color: '#7c3aed', inset: false };
...
export function BoxShadowGenerator() {
   ...
   const tailwindShadowValue = useMemo(() => {
      return shadows
         .map((s) => `${s.inset ? 'inset_' : ''}${s.x}px_${s.y}px_${s.blur}px_${s.spread}px_${s.color.replace(/\s+/g, '')}`)
         .join(',');
   }, [shadows]);
         ...
         <div className="md:w-[320px] shrink-0 overflow-y-auto border-r border-border/30 p-4 space-y-4">
            ...
            {shadows.map((shadow, i) => (
               <ControlGroup key={i} title={`Shadow ${i + 1}`}>
                  <ControlSlider label="Offset X" value={shadow.x} onChange={(v) => updateShadow(i, 'x', v)} min={-50} max={50} unit="px" />
                  <ControlSlider label="Offset Y" value={shadow.y} onChange={(v) => updateShadow(i, 'y', v)} min={-50} max={50} unit="px" />
                  <ControlSlider label="Blur" value={shadow.blur} onChange={(v) => updateShadow(i, 'blur', v)} min={0} max={100} unit="px" />
                  <ControlSlider label="Spread" value={shadow.spread} onChange={(v) => updateShadow(i, 'spread', v)} min={-50} max={50} unit="px" />
                  ...
                  {shadows.length > 1 && (
                     <Button variant="ghost" size="sm" className="h-7 text-xs text-destructive/70 hover:text-destructive w-full" onClick={() => removeShadow(i)}>
