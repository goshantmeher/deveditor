...
.grid-item:hover {
  transform: scale(1.05);
   ...
   const html = Array.from({ length: clampedItems }, (_, i) =>
      ...
      <div className="flex flex-col md:flex-row h-full min-h-0">
         <div className="md:w-[320px] shrink-0 overflow-y-auto border-r border-border/30 p-4 space-y-4">
            ...
            <ControlGroup title="Column Size">
               <ControlSelect label="Unit" value={colUnit} onChange={setColUnit} options={[
                  { value: '1fr', label: '1fr' },
            ...
            <ControlGroup title="Row Size">
               <ControlSelect label="Unit" value={rowUnit} onChange={setRowUnit} options={[
                  { value: 'auto', label: 'Auto' },
                  { value: '1fr', label: '1fr' },
               ...
               {rowUnit === 'auto' && (
                  <ControlSlider label="Min Height" value={itemSize} onChange={setItemSize} min={40} max={200} unit="px" />
               )}
            </ControlGroup>
         </div>

         <div className="flex-1 min-h-0">
            <GeneratorPreview css={css} html={`<div class="grid-container">\n  ${html}\n</div>`} tailwind={tailwind} bodyBg={bodyBg} previewStyle={{ background: bodyBg }} />
         </div>
      </div>
   );
}
