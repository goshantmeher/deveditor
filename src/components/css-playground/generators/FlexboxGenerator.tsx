      ...
      <div className="flex flex-col md:flex-row h-full min-h-0">
         <div className="md:w-[320px] shrink-0 overflow-y-auto border-r border-border/30 p-4 space-y-4">
            ...
            <ControlGroup title="Flex Container">
               <ControlSelect label="Direction" value={direction} onChange={setDirection} options={[
                  { value: 'row', label: 'Row' },
                  { value: 'row-reverse', label: 'Row Reverse' },
                  { value: 'column', label: 'Column' },
                  { value: 'column-reverse', label: 'Col Reverse' },
               ]} />
               <ControlSelect label="Wrap" value={wrap} onChange={setWrap} options={[
                  { value: 'nowrap', label: 'No Wrap' },
                  { value: 'wrap', label: 'Wrap' },
                  { value: 'wrap-reverse', label: 'Wrap Reverse' },
               ]} />
               <ControlSelect label="Justify Content" value={justifyContent} onChange={setJustifyContent} options={[
               ...
               <ControlSelect label="Align Items" value={alignItems} onChange={setAlignItems} options={[
            ...
            <ControlGroup title="Flex Items">
               ...
               <div className="flex gap-2">
                  ...
                  {itemCount > 1 && (
                     <Button variant="ghost" size="sm" className="w-full h-8 text-xs text-destructive hover:text-destructive" onClick={removeItem}>
         ...
         <div className="flex-1 min-h-0">
            <GeneratorPreview css={css} html={`<div class="container">\n  ${html}\n</div>`} tailwind={tailwind} bodyBg={bodyBg} previewStyle={{ background: bodyBg }} />
         </div>
      </div>
   );
}
