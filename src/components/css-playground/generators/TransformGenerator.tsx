...
export function TransformGenerator() {
   ...
   const transforms = [
      ...
      scaleX !== 100 || scaleY !== 100 ? `scale(${(scaleX / 100).toFixed(2)}, ${(scaleY / 100).toFixed(2)})` : '',
   ...
   const tailwindTransform = transformValue !== 'none' ? ` [transform:${transformValue.replace(/ /g, '_')}]` : '';
      ...
      <div className="flex flex-col md:flex-row h-full min-h-0">
         <div className="md:w-[320px] shrink-0 overflow-y-auto border-r border-border/30 p-4 space-y-4">
            ...
            <ControlGroup title="Box">
               <ControlSlider label="Size" value={boxSize} onChange={setBoxSize} min={60} max={250} unit="px" />
            ...
            <ControlGroup title="Rotate">
               <ControlSlider label="X" value={rotateX} onChange={setRotateX} min={-180} max={180} unit="°" />
               <ControlSlider label="Y" value={rotateY} onChange={setRotateY} min={-180} max={180} unit="°" />
               <ControlSlider label="Z" value={rotateZ} onChange={setRotateZ} min={-180} max={180} unit="°" />
            </ControlGroup>

            <ControlGroup title="Scale">
               <ControlSlider label="X" value={scaleX} onChange={setScaleX} min={10} max={200} unit="%" />
               <ControlSlider label="Y" value={scaleY} onChange={setScaleY} min={10} max={200} unit="%" />
            </ControlGroup>

            <ControlGroup title="Skew">
               <ControlSlider label="X" value={skewX} onChange={setSkewX} min={-45} max={45} unit="°" />
               <ControlSlider label="Y" value={skewY} onChange={setSkewY} min={-45} max={45} unit="°" />
            </ControlGroup>

            <ControlGroup title="Translate">
               <ControlSlider label="X" value={translateX} onChange={setTranslateX} min={-100} max={100} unit="px" />
               <ControlSlider label="Y" value={translateY} onChange={setTranslateY} min={-100} max={100} unit="px" />
            </ControlGroup>

            <ControlGroup title="Perspective">
               <ControlSlider label="Distance" value={perspective} onChange={setPerspective} min={100} max={2000} unit="px" />
            </ControlGroup>
         </div>
