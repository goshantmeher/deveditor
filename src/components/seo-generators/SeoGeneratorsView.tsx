...
export function SeoGeneratorsView() {
   ...
   const generateRobotsTxt = () => {
      ...
      const allows = state.robotsAllow.split('\n').map(l => l.trim()).filter(Boolean);
      allows.forEach(a => { output += `Allow: ${a}\n`; });
      
      const disallows = state.robotsDisallow.split('\n').map(l => l.trim()).filter(Boolean);
      disallows.forEach(d => { output += `Disallow: ${d}\n`; });
   ...
   const generateSitemapXml = () => {
      ...
      const paths = state.sitemapPaths.split('\n').map(l => l.trim()).filter(Boolean);
      ...
      <div className="w-full h-full flex flex-col md:flex-row bg-background overflow-hidden relative border border-border">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6">
                  ...
                  <>
                     <div className="space-y-4">
                        <div className="space-y-1.5">
                           <label className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Target User-Agent</label>
                           <Select value={state.robotsUserAgent} onValueChange={(val) => handleChange('robotsUserAgent', val)}>
                        ...
                        <div className="space-y-1.5">
                           <label className="text-[11px] font-medium text-emerald-500 uppercase tracking-wider">Allowed Paths (One per line)</label>
                           <Textarea 
                        ...
                        <div className="space-y-1.5">
                           <label className="text-[11px] font-medium text-rose-500 uppercase tracking-wider">Disallowed Paths (One per line)</label>
                           <Textarea 
                        ...
                        <div className="space-y-1.5 pt-4 border-t border-border">
                           <label className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Sitemap XML URL</label>
                           <Input 
                              type="url"
               ...
               {state.activeTab === 'sitemap' && (
                  <>
                     <div className="space-y-4">
                        <div className="space-y-1.5">
                           <label className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Base Domain URL</label>
                           <Input 
                              type="url"
                        ...
                        <div className="grid grid-cols-2 gap-4">
                           <div className="space-y-1.5">
                              <label className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Change Freq.</label>
                              <Select value={state.sitemapFreq} onValueChange={(val) => handleChange('sitemapFreq', val)}>
                           ...
                           <div className="space-y-1.5">
                              <label className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Priority (0.1-1.0)</label>
                              <Select value={state.sitemapPriority} onValueChange={(val) => handleChange('sitemapPriority', val)}>
                                 <SelectTrigger className="w-full bg-background border border-border rounded-md px-3 text-sm">
                        ...
                        <div className="space-y-1.5 pt-2">
                           <label className="text-[11px] font-medium text-indigo-400 uppercase tracking-wider">Routing Paths (One per line)</label>
                           <Textarea 
         ...
         <div className="flex-1 flex flex-col min-w-0 bg-[#1e1e1e] h-[50vh] md:h-full relative overflow-hidden">
            <div className="h-14 flex items-center justify-between px-4 border-b border-white/10 shrink-0 absolute top-0 left-0 right-0 z-10 w-full bg-[#181818] shadow-md">
               ...
               <Button variant="outline" size="sm" className="h-8 gap-2 bg-transparent text-gray-300 border-white/20 hover:bg-white/10 hover:text-white" onClick={copyToClipboard}>
            ...
            <div className="flex-1 w-full pt-14 overflow-auto p-4 md:p-8">
               <pre className="text-[13px] md:text-[14px] font-mono leading-relaxed text-[#c9d1d9] bg-transparent whitespace-pre">
                  <code dangerouslySetInnerHTML={{ __html: currentOutput.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br/>') }} />
               </pre>
            </div>
         </div>
      </div>
